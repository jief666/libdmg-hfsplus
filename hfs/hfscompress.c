...
void flipHFSPlusDecmpfs(HFSPlusDecmpfs* compressData) {
	...
	FLIPENDIANLE(compressData->uncompressed_size);
}
...
static size_t compressedRead(io_func* io, off_t location, size_t size, void *buffer) {
...
static int compressedWrite(io_func* io, off_t location, size_t size, void *buffer) {
	...
	if(data->cachedStart != 0 || data->cachedEnd != data->decmpfs->uncompressed_size) {
		// Cache entire file
		uint8_t* newCache = (uint8_t*) malloc(data->decmpfs->uncompressed_size);
		compressedRead(io, 0, data->decmpfs->uncompressed_size, newCache);
		if(data->cached)
			free(data->cached);

		data->cached = newCache;
		data->cachedStart = 0;
		data->cachedEnd = data->decmpfs->uncompressed_size;
	}

	if((location + size) > data->decmpfs->uncompressed_size) {
		data->decmpfs->uncompressed_size = location + size;
		data->cached = (uint8_t*) realloc(data->cached, data->decmpfs->uncompressed_size);
		data->cachedEnd = data->decmpfs->uncompressed_size;
	}
...
static void closeHFSPlusCompressed(io_func* io) {
	...
	if(data->dirty) {
		...
		uint32_t numBlocks = (data->decmpfs->uncompressed_size + 0xFFFF) / 0x10000;
		...
		for(i = 0; i < numBlocks; i++) {
			...
			compress(buffer, &actualSize, data->cached + (0x10000 * i),
					(data->decmpfs->uncompressed_size - (0x10000 * i)) > 0x10000 ? 0x10000 : (data->decmpfs->uncompressed_size - (0x10000 * i)));
...
io_func* openHFSPlusCompressed(Volume* volume, HFSPlusCatalogFile* file) {
    ...
    data->decmpfsSize = getAttribute(volume, file->fileID, "com.apple.decmpfs", (uint8_t**)(&data->decmpfs), 1);
    flipHFSPlusDecmpfs(data->decmpfs);
...
//    if(data->decmpfs->size < 16) {
//        data->cached = (uint8_t*)data->decmpfs->data;
//        data->cachedEnd = data->decmpfs->size;
//    }
//    else
    if(data->decmpfs->flags == 0x3) {
        data->cached = (uint8_t*) malloc(data->decmpfs->uncompressed_size);
        actualSize = data->decmpfs->uncompressed_size;
        if ( (data->decmpfs->data[0] & 0x0f) == 0x0f ) {
            if ( actualSize != data->decmpfsSize - 16 - 1 ) {
                hfs_panic("actualSize != data->decmpfsSize - 16 - 1");
            }
		...
		if(actualSize != data->decmpfs->uncompressed_size) {
			hfs_panic("decmpfs: size mismatch");
		}
		data->cachedStart = 0;
